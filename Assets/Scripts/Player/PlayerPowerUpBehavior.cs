...
public class PlayerPowerUpBehavior : MonoBehaviour
{
    ...
    {
        if(collector.GetCurrentPowerUp() == Collector.powers.wireFrame)
        {
            playerWF.SetWireFrame(true);

            timeToFinish = Time.time + 5.0f;

        }else if(collector.GetCurrentPowerUp() == Collector.powers.shoot)
        {
            playerShoot.SetDamage(initDamage*4);
            timeToFinish = Time.time + 5.0f;

        }else if(collector.GetCurrentPowerUp() == Collector.powers.speed)
        {
